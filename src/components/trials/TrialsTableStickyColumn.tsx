...
// New container for the table and buttons side by side
const TableWithButtonsContainer = styled.div`
  display: flex;
...
const DeleteButtonsContainer = styled.div`
  ...
  padding-left: 8px;
  /* No need for static padding-top - container will start with placeholder */
`;

// Button placeholder for header (empty space)
const ButtonPlaceholder = styled.div<{ height: string }>`
  height: ${props => props.height}; /* Match the header height */
`;
...
const TrialsTable = forwardRef<HTMLDivElement, TrialsTableProps>(({
  ...
  // Add state to track row heights
  const [rowHeights, setRowHeights] = useState<Record<number, string>>({});
  // Ref to observe row height changes
  const rowsRef = useRef<Map<number, HTMLTableRowElement>>(new Map());
  // Add a ref for the header row
  const headerRowRef = useRef<HTMLTableRowElement | null>(null);
  // Add state to track header height
  const [headerHeight, setHeaderHeight] = useState<string>('42px');
  ...
  useEffect(() => {
    ...
    // Function to check if scrolled to the end
    const handleScroll = () => {
  ...
  const updateRowHeights = useCallback(() => {
    ...
    if (headerRowRef.current) {
      setHeaderHeight(`${headerRowRef.current.offsetHeight}px`);
    }
    ...
    rowsRef.current.forEach((row, trialId) => {
      if (row) {
        newHeights[trialId] = `${row.offsetHeight}px`;
      }
    });
  ...
  // Update row heights when trials change
  useEffect(() => {
  ...
  }, [trials, updateRowHeights]);

  // Create a resize observer to update row heights when window resizes
  useEffect(() => {
    ...
    const tableContainer = document.querySelector('.table-rows-container');
    if (tableContainer) {
      ...
      <AnimatedTableContainer>
        <TableWithButtonsContainer>
                      ...
                      {trials.map((trial) => {
                        ...
                        return (
                          <TableRow 
                            ...
                            ref={row => {
                              if (row) {
                                rowsRef.current.set(trial.id, row);
                              } else {
                                rowsRef.current.delete(trial.id);
                              }
                            }}
                          >
          ...
          <DeleteButtonsContainer>
            {/* Empty placeholder for the header row - with dynamic height */}
            <ButtonPlaceholder height={headerHeight} />
            ...
            {trials.length === 0 ? (
              ...
              // Render delete buttons for each trial
              <>
                {trials.map((trial) => {
                  // Skip deleted rows
                  if (trial.id === deletedRowId) {
                    return (
                      <DeleteButtonRow 
                        key={`delete-placeholder-${trial.id}`} 
                        height={`${rowHeightRef.current}px`}
                      />
                    );
                  }
                  
                  // Render delete button aligned with its row
                  return (
                    <DeleteButtonRow 
                      key={`delete-${trial.id}`}
